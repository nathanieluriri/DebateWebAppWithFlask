...
from Controller.getClaimsFromTopicId import get_claim_count_per_topic, get_claims_for_topic
...
from Controller.reply import get_reply_type_id, insert_reply, insert_reply_to_claim, insert_reply_to_reply
...
def create_claim_relationship():
    ...
    try:
        ...
        if not data or 'first' not in data or 'claimRelType' not in data:
            return jsonify({"error": "Missing required fields: first or claimRelType"}), 400
        ...
        success, error_message = insert_claim_to_claim(first_claim_id, second_claim_id, claim_rel_type)
...
def create_user():
    ...
    try:
        user_id =insert_user_to_db(userName, passwordHash, isAdmin, creationTime, lastVisit)
...
def create_claim():
    ...
    try:
        ...
        return jsonify({'message': 'Claim created successfully'}), 201  # 201 Created status
    except Exception as e:
        return jsonify({'error': str(e)}), 500  # Return error if something goes wrong
...
def create_reply():
    ...
    if reply_type_id is None:
        return jsonify({'error': f'Invalid relationship type: {relationship_type} for {reply_type}'}), 400
